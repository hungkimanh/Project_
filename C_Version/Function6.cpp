...
static inline void hash_combine_u64(std::uint64_t &seed, std::uint64_t v) {
    seed ^= splitmix64(v + 0x9e3779b97f4a7c15ULL + (seed << 6) + (seed >> 2));
}

static std::uint64_t solution_signature(const Solution &s) {
    std::uint64_t seed = 1469598103934665603ULL;
    hash_combine_u64(seed, (std::uint64_t)s.trucks.size());
    for (const auto &tr : s.trucks) {
        hash_combine_u64(seed, (std::uint64_t)tr.truck_id);
        hash_combine_u64(seed, (std::uint64_t)tr.stops.size());
        for (const auto &st : tr.stops) {
            hash_combine_u64(seed, (std::uint64_t)(st.customer + 1000003));
            hash_combine_u64(seed, (std::uint64_t)st.loaded_from_drone.size());
            for (auto pkg : st.loaded_from_drone) {
                hash_combine_u64(seed, (std::uint64_t)(pkg + 1000033));
            }
        }
    }
    hash_combine_u64(seed, (std::uint64_t)s.drone_queue.size());
    for (const auto &trip : s.drone_queue) {
        hash_combine_u64(seed, (std::uint64_t)trip.events.size());
        for (const auto &ev : trip.events) {
            hash_combine_u64(seed, (std::uint64_t)(ev.rendezvous_customer + 1000211));
            hash_combine_u64(seed, (std::uint64_t)(ev.truck_id + 1000249));
            hash_combine_u64(seed, (std::uint64_t)ev.packages.size());
            for (auto pkg : ev.packages) {
                hash_combine_u64(seed, (std::uint64_t)(pkg + 1000289));
            }
        }
    }
    return seed;
...
static bool remember_solution_signature(
    std::unordered_set<std::uint64_t> &seen,
    std::deque<std::uint64_t> &order,
    std::uint64_t sig,
    std::size_t cap
) {
    auto ins = seen.insert(sig);
    if (!ins.second) return false;
    order.push_back(sig);
    if (cap > 0 && seen.size() > cap) {
        std::uint64_t old = order.front();
        order.pop_front();
        seen.erase(old);
    }
    return true;
...
static NeighborhoodApplyResult truck_move_1_0_best(
    ...
    if (found) {
        ...
        normalize_after_transform(p, best_sol);
        s = std::move(best_sol);
    }
    return out;
}
...
static void tabu_search_truck(const Params &p, Solution &s) {
    ...
    for (int it = 0; it < MAX_IT; ++it) {
        ...
        if (!found) break;
        // apply best move
        normalize_after_transform(p, cur_best);
        s = std::move(cur_best);
    ...
    normalize_after_transform(p, best);
    s = std::move(best);
}
...
static void ats_full(const Params &p, Solution &s, int SEG = 4, double theta = 2.0, int DIV = 3) {
    ...
    const std::size_t ls_light_seen_cap = (std::size_t)std::max(0, env_int("LS_LIGHT_SEEN_CAP", 200000));
    std::unordered_set<std::uint64_t> ls_light_seen;
    std::deque<std::uint64_t> ls_light_seen_order;
    int ls_light_skip_count = 0;
    ...
    while (div_no_improve < DIV) {
        ...
        while (seg_no_improve < SEG) {
            ...
            while (no_imp_iter < NIMP) {
                ...
                // Skip if this exact state has already been refined before.
                if (remember_solution_signature(
                        ls_light_seen,
                        ls_light_seen_order,
                        solution_signature(cur),
                        ls_light_seen_cap
                    )) {
    ...
    std::cout << "[ATS] light-LS repeated-state skips: " << ls_light_skip_count << "\n";
...
static bool read_seed_solution_file(const Params &p, const std::string &path, Solution &sol, std::string &reason) {
    ...
    rebuild_loaded_from_drone_marks(p, cand);
    normalize_after_transform(p, cand);
    std::string why;
...
static void repair_segment_precedence_same_truck(const Params &p, Solution &sol, int truck_id, int pos_lo, int pos_hi) {
    ...
    auto try_assign_pkg_same_truck = [&](int pkg) -> bool {
        ...
        if (p.customers[pkg].release <= 0) return false;
...
static void normalize_after_transform(const Params &p, Solution &sol) {
    ...
    // owner/position maps on current truck routes
    std::vector<int> owner_truck(n, -1), owner_pos(n, -1);
    for (size_t t = 0; t < sol.trucks.size(); ++t) {
        for (size_t pos = 0; pos < sol.trucks[t].stops.size(); ++pos) {
            int c = sol.trucks[t].stops[pos].customer;
    ...
    // Repeat until stable because removing resupply packages can increase rmax_k.
    bool changed = true;
    while (changed) {
        changed = false;

        // Stage 1: basic filtering + precedence/ownership
        for (auto &trip : sol.drone_queue) {
            std::vector<ResupplyEvent> kept_events;
            kept_events.reserve(trip.events.size());
            for (auto &ev : trip.events) {
                int rv = ev.rendezvous_customer;
                int t = (ev.truck_id >= 0 && ev.truck_id < (int)sol.trucks.size())
                            ? ev.truck_id
                            : -1;
                int rv_owner = (rv > 0 && rv < n) ? owner_truck[rv] : -1;
                int rv_pos = (t >= 0 && rv > 0 && rv < n) ? owner_pos[rv] : -1;
                if (t < 0 || rv_owner != t || rv_pos < 0) { changed = true; continue; }

                std::vector<int> pk;
                pk.reserve(ev.packages.size());
                for (int x : ev.packages) {
                    // Function6 policy: release<=0 packages must stay on truck from depot.
                    if (x <= 0 || x >= n) { changed = true; continue; }
                    if (p.customers[x].release <= 0) { changed = true; continue; }
                    if (owner_truck[x] != t) { changed = true; continue; }
                    if (owner_pos[x] < rv_pos) { changed = true; continue; }
                    pk.push_back(x);
                }
                if (pk.empty()) { changed = true; continue; }
                ev.packages.swap(pk);
        ...
        // Stage 2: remove empty trips/events first
        {
            std::vector<DroneTrip> kept_trips;
            kept_trips.reserve(sol.drone_queue.size());
            for (auto &trip : sol.drone_queue) {
            ...
            if (kept_trips.size() != sol.drone_queue.size()) changed = true;
            sol.drone_queue.swap(kept_trips);
        }

        // Stage 3: enforce per-truck rmax_k rule
        std::vector<char> is_resup(n, 0);
        for (const auto &trip : sol.drone_queue) {
            for (const auto &ev : trip.events) {
                for (int x : ev.packages) if (x > 0 && x < n) is_resup[x] = 1;
            }
        }

        std::vector<int> rmax(sol.trucks.size(), 0);
        for (int c = 1; c < n; ++c) {
            int t = owner_truck[c];
            if (t < 0) continue;
            if (!is_resup[c]) rmax[t] = std::max(rmax[t], p.customers[c].release);
        }

        for (auto &trip : sol.drone_queue) {
            for (auto &ev : trip.events) {
                int t = ev.truck_id;
                if (t < 0 || t >= (int)rmax.size()) continue;
                std::vector<int> kept_pk;
                kept_pk.reserve(ev.packages.size());
                for (int x : ev.packages) {
                    // If release(x) <= rmax_k then truck k must take x at depot.
                    if (p.customers[x].release <= rmax[t]) { changed = true; continue; }
                    kept_pk.push_back(x);
                }
                ev.packages.swap(kept_pk);
            }
        }

        // Stage 4: cleanup after rmax filtering
        for (auto &trip : sol.drone_queue) {
            std::vector<ResupplyEvent> kept_events;
            kept_events.reserve(trip.events.size());
            for (auto &ev : trip.events) {
                if (!ev.packages.empty()) kept_events.push_back(std::move(ev));
                else changed = true;
            }
            trip.events.swap(kept_events);
        }
...
static bool relocate_sync_point_first_improve(const Params &p, Solution &s) {
    ...
    if (improved_any) { normalize_after_transform(p, best_sol); s = std::move(best_sol); return true; }
    return false;
}
...
static bool relocate_package_first_improve(const Params &p, Solution &s) {
    ...
    for (int k = 0; k < p.n_truck; ++k) {
        ...
        for (size_t pos = 1; pos + 1 < s.trucks[k].stops.size(); ++pos) {
            ...
            if (p.customers[pkg].release <= 0) continue;
    ...
    for (const auto &c : cands) {
        ...
        for (size_t pos2 = pos_del+1; pos2 + 1 < s.trucks[k].stops.size(); ++pos2) {
            ...
            if (p.customers[pk2].release <= 0) continue;
    ...
    for (size_t trip_idx = 0; trip_idx < s.drone_queue.size(); ++trip_idx) {
        for (size_t ev_idx = 0; ev_idx < s.drone_queue[trip_idx].events.size(); ++ev_idx) {
            ...
            for (size_t pkg_idx = 0; pkg_idx < ev.packages.size(); ++pkg_idx) {
                ...
                if (pkg <= 0 || pkg >= (int)p.customers.size()) continue;
                if (p.customers[pkg].release <= 0) continue;
...
int main(int argc, char** argv){
    ...
    if (!skip_ats) {
        ...
        std::cout << "[ATS] skipped by SKIP_ATS\n";
		    ...
		    const std::size_t ls_strong_seen_cap = (std::size_t)std::max(0, env_int("LS_STRONG_SEEN_CAP", 200000));
		    std::unordered_set<std::uint64_t> ls_strong_seen;
		    std::deque<std::uint64_t> ls_strong_seen_order;
		    int ls_strong_skip_count = 0;
		    int shake_cnt = 0;
		    std::mt19937 rng{std::random_device{}()};
		    ...
		    while (!solver_time_limit_reached()) {
		        ...
		        if (!remember_solution_signature(
		                ls_strong_seen,
		                ls_strong_seen_order,
		                solution_signature(sol),
		                ls_strong_seen_cap
		            )) {
		            ls_strong_skip_count++;
		    ...
		    std::cout << "[LS] repeated-state skips: " << ls_strong_skip_count << "\n";
