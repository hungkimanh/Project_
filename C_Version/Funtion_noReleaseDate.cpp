...
// No-release-date variant requested: disable all drone-resupply mechanics.
static constexpr bool kDisableDroneResupply = true;
        ...
        for (size_t i = 1; i + 1 < s.trucks[t1].stops.size(); ++i) {
            for (int t2 = t1; t2 < K; ++t2) {
                ...
                for (size_t j = j_start; j + 1 < s.trucks[t2].stops.size(); ++j) {
                    ...
                    if (!kDisableDroneResupply && t1 == t2) {
...
static void apply_drone_local_search(const Params &p, Solution &s) {
    if (kDisableDroneResupply) return;
...
static void ats_full(const Params &p, Solution &s, int SEG = 4, double theta = 2.0, int DIV = 3) {
    ...
    while (div_no_improve < DIV) {
        ...
        while (seg_no_improve < SEG) {
            ...
            while (no_imp_iter < NIMP) {
                ...
                // local search on truck result: in no-release mode we keep drone resupply disabled.
                if (!kDisableDroneResupply) {
                    apply_drone_local_search(p, cur);
                }
                if (!kDisableDroneResupply && first_iter_segment) {
    ...
    if (!kDisableDroneResupply && !stopped_by_time) {
...
static void read_instance(const string &path, Params &p){
    ...
    auto parse_customer = [&](const string &ln){
        ...
        c.demand = stoi(tok[2]);
        // No-release-date variant: force every package release date to 0.
        c.release = 0;
...
int main(int argc, char** argv){
    ...
    if (!kDisableDroneResupply) {
        ...
        std::cout << "[LS] drone resupply is disabled in this build\n";
    }
